...
class Weibo(object):
    def __init__(self, config):
        """Weibo类初始化"""
        self.start_num= config["start_num"]
        self.spider_weibo_id_list_file= config["spider_weibo_id_queue_file"]
        self.spider_weibo_id_list=[] #设置爬取的微博队列
        self.get_spider_weibo_id_list()
    ...
    def get_spider_weibo_id_list(self):
        df=pd.read_csv("D:\myspider\code\weibo-crawler-master\weibo-crawler-master\\"+self.spider_weibo_id_list_file,index_col=False)
        print(df.head())
        self.spider_weibo_id_list=df["id"].values.tolist()[self.start_num:]
        print("现在共有"+str(len(self.spider_weibo_id_list))+"条数据需要爬取")
    ...
    def write_json(self, wrote_count):
        ...
        data["weibo"] = [w for w in data['weibo'] if w["video_url"] == '']  # 去掉有视频的博文
    ...
    def parse_sqlite_repost(self, repost, weibo):
        ...
        sqlite_repost["created_at"]=self.standardize_date(sqlite_repost["created_at"])[1]
        sqlite_repost["weibo_id"] = str(weibo)
    ...
    def get_sqlte_path(self):
        return "weibo/test-digui-weibodata.db"
    ...
    def my_get_weibo_reposts_cookie(
            self,  cur_count, max_count, page, on_downloaded, spider_weibo_id_queue
    ):
        """
        :weibo standardlized weibo
        :cur_count  已经下载的转发数
        ...
        url = "https://m.weibo.cn/api/statuses/repostTimeline"
        #id=weibo["id"]
        id=spider_weibo_id_queue.popleft()
        params = {"id": id, "page": page}
        try:
            req = requests.get(
                url,
            ...
            reposts = data.get("data")
            count = len(reposts)
            print("repost",reposts)
            print("count is",count)
            if count == 0:
                # 没有了可以直接跳出递归
                logger.info(f"微博id: {id} 共抓取{count}".format(id=id,count=count))
                return
            
            for re in reposts:
                spider_weibo_id_queue.append(re["id"])
                
            if on_downloaded:
                on_downloaded(id, reposts)
            
            if len(spider_weibo_id_queue)==0:
                logger.info(f"微博id: {id} 共抓取{count}".format(id=id,count=count))
                return 
            ...
            if page > req_page:
                return
            self.my_get_weibo_reposts_cookie(cur_count, max_count, page, on_downloaded,spider_weibo_id_queue)
        except Exception as e:
            logger.warning(
                f"未能抓取完整转发 微博id: {id}".format(id=id)
            )
            return
       

    def mystart(self):
        #self.sqlite_insert_reposts
        #self.initialize_info(user_config)
        
        user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36"
        self.headers = {"User_Agent":  user_agent, "Cookie": self.cookie[0]}
        for i in tqdm(range(len(self.spider_weibo_id_list))):
            try:
                # 这里在读取下一个用户的时候很容易被ban，需要优化休眠时长
                # 加一个count，不需要一上来啥都没干就sleep
                weibo_id=self.spider_weibo_id_list[i]
                now_weibo_queue=deque()
                now_weibo_queue.append(weibo_id)

                self.my_get_weibo_reposts_cookie(0,self.repost_max_download_count,1,self.sqlite_insert_reposts,now_weibo_queue)
                if i % 5 == 0:
                    self.headers = {"User_Agent":  user_agent, "Cookie": self.cookie[i%2]}
                    logger.info("{} finish".format(i+self.start_num))
                    if self.long_sleep_count_before_each_user > 0:
                ...
                logger.info(f"{id} has problem".format(id=weibo_id))
                logger.exception(e)
...
def get_config():
    """获取config.json文件信息"""
    config_path = os.path.split(os.path.realpath(__file__))[0] + os.sep + "config_copy.json"
