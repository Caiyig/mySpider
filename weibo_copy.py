...
class Weibo(object):
    def __init__(self, config):
        """Weibo类初始化"""
        self.min_weibo_num=5  #发表推文的最小量 如果这段时间发表+转发的推文量小于这个值 则不保存该用户的信息
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if retweeted_status and retweeted_status.get("id"):  # 转发
                ...
                """
                if weibo["reposts_count"]==0:  #cyy 被转发数量为0  不保存该微博 但是应该对发表量+1 考虑后续可能会增加对于原创微博的兴趣 暂时保留[转发量为0 & 原创的微博]
                    return None
                """
    ...
    def parse_sqlite_repost(self, repost, weibo):
        ...
        if "pid" not in repost.keys(): #转发的上一层id
            sqlite_repost["weibo_id"] = weibo["id"]
        else:
            sqlite_repost["weibo_id"] = repost["pid"]
        sqlite_repost["init_weibo_id"] = weibo["id"] #原创微博
    ...
    def get_sqlte_path(self):
        return "weibo/weibodata_1-1_test.db"
    ...
    def get_pages(self):
        ...
        try:
            ...
            if since_date <= today:    # since_date 若为未来则无需执行
                ...
                ## cyy 如果发表推文数小于某个值 则不保存
                if self.got_count<self.min_weibo_num:
                    logger.info("删除该用户{}".format(self.user_config["user_id"]))
                else:
                    self.write_data(0)  # 将所有的保存
